...
> 구조화된 워크플로우는 [MCP 프롬프트](#mcp-프롬프트-슬래시-커맨드) 참고.
...
| 프롬프트 이름 | 설명 |
|--------------|------|
| `verify` | 완전한 Verifier↔Critic 루프 실행 |
| `consolidate` | 우선순위화된 수정 계획 생성 |
| `apply` | 검증과 함께 수정 적용 |
...
<details>
<summary><strong>시스템 다이어그램</strong></summary>
...
**해결책:**
1. 클라이언트의 MCP 설정에서 설치 확인
2. 서버 추가 후 MCP 클라이언트 재시작
